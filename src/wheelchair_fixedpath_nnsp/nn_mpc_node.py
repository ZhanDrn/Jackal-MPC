...
def main():
    ...
    caster_sub = Gazebo_caster_state()
    ...
    while not rospy.is_shutdown():
        ...
        #s=s0
        #if iter%50==0:
        s = FindS(csX,csY,pos[0],pos[1],s0-2,s0+2,0.1)
        
        #s = x0
        xe, ye ,the = RefToS(csX,csY,pos[0],pos[1],pos[2],s)
        x0 = np.array([xe, ye, the, x[3], x[4],s, caster_rot[0]+np.pi/2, caster_rot[1]+np.pi/2])
        ...
        for i in range(N):
            ...
            acados_solver.set(i, "p", np.hstack((params[i],K)))
        ...
        #print("Casters: ", caster_rot[0], caster_rot[1])
