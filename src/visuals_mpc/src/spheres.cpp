    ...
    void costmap_calback(const nav_msgs::OccupancyGrid::ConstPtr& msg){
        ...
        for (int i=0;i<length;i++) {
            if (msg->data[i]>99.0) {
                ...
                costmap_data[2*obst_count+0] = (i % matrix_size)*resolution+msg->info.origin.position.x;
                costmap_data[2*obst_count+1] = ((int)i / matrix_size)*resolution+msg->info.origin.position.y;
