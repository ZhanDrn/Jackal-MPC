...
def skid_model(learned_dyn, N_obst):
    ...
    p = vertcat(nnp,c,obsX,obsY,obsS)
    #parameter_values = np.array([0])
    parameter_values = np.append(learned_dyn.approx_params(np.ones([5]), flat=True, order=1),np.zeros([1+3*N_obst]))
    ...
    obs = exp(-3*((obsX-x)**2+(obsY-y)**2+(obsS-s)**2))
    #obs = exp(-3*((obsX-x)**2+(obsY-y)**2))
