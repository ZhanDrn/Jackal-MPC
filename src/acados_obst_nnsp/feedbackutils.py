...
class CostmapPath:
    ...
    def findObst1(self,N=10):
        obs_indexes = np.argwhere(self.costmap>99)
        obsX = np.zeros(N)-1000
        obsY = np.zeros(N)-1000
        if obs_indexes.shape[0]<=1:
            return obsX, obsY
        H = self.height//2
        W = self.width//2
        diff = abs(obs_indexes-np.array([H, W]))
        dist = diff[:,0]**2+diff[:,1]**2
        if dist.shape[0]<N:
            N = dist.shape[0]
        indexes = obs_indexes[np.argpartition(dist,N-1)[:N-1],:]
        obs_rows = indexes[:,0]
        obs_cols = indexes[:,1]
        obsY[:obs_rows.shape[0]] = (obs_rows)*self.resolution+self.originY
        obsX[:obs_cols.shape[0]] = (obs_cols)*self.resolution+self.originX
        return obsX, obsY
    def findObst1(self,N=10, spline_xe = 0, spline_ye = 0):
        obs_indexes = np.argwhere(self.costmap>99)
        obsX = np.zeros(N)-1000
        obsY = np.zeros(N)-1000
        if obs_indexes.shape[0]<=1:
        ...
        H = self.height//2
        W = self.width//2
        X_idx_offset =  spline_xe//self.resolution
        Y_idx_offset =  spline_ye//self.resolution
        X_idx_offset = -W if X_idx_offset < -W else X_idx_offset
        X_idx_offset =  W if X_idx_offset >  W else X_idx_offset
        Y_idx_offset = -H if Y_idx_offset < -H else Y_idx_offset
        Y_idx_offset =  H if Y_idx_offset >  H else Y_idx_offset
        X_idx = W+X_idx_offset
        Y_idx = H+Y_idx_offset

        diff = abs(obs_indexes-np.array([X_idx, Y_idx]))
        dist = diff[:,0]**2+diff[:,1]**2
        if dist.shape[0]<N:
            N = dist.shape[0]
        indexes = obs_indexes[np.argpartition(dist,N-1)[:N-1],:]
