...
def main():
    ...
    #Solver Setup
    Tf = 2*3.5  # prediction horizon
    N = 2*70  # number of discretization steps
    N_obst = 260
    r=0.11
    Ts = Tf/N
    ...
    V_target = 0.5
    x_l = []
    csX = CubicSpline(path_sub.s, path_sub.x)
    csY = CubicSpline(path_sub.s, path_sub.y)
    iter=0
    freq = 20
    rate = rospy.Rate(freq) # hzs
    while not rospy.is_shutdown():
        iter+=1
        #print(s_plan.shape)
        #print(s_plan, dists, idx_plan)
        pos = pos_sub.pos
        s0 = np.array(acados_solver.get(0, "x"))[5]
        #s = s0
        s = FindS(csX,csY,pos[0],pos[1],s0-5,s0+5,0.1)
        if iter%(2*freq) == 0:
            s = FindS(csX,csY,pos[0],pos[1],s0-5,s0+5,0.1)
        ...
        #obsX, obsY = costmap_sub.findObst1(N=N_obst)
        for i in range(N):
            ...
            obsX, obsY = costmap_sub.findObst1(N=N_obst,spline_xe=csX(x_l[i,5])-csX(x_l[0,5])-xe,spline_ye=csY(x_l[i,5])-csY(x_l[0,5])-ye)
            #print(obsX)
            #print(obsY)
            #acados_solver.set(i, "p", K)
            obsS = np.repeat(x_l[i,5],N_obst)
            obsXe, obsYe = ObsToS(csX,csY,obsX,obsY,obsS)
            acados_solver.set(i, "p", np.hstack((params[i],K,obsXe,obsYe,obsS)))
        status = acados_solver.solve()
        x = np.array(acados_solver.get(1, "x"))
